.TH "Dokumentacja katalogu C:/PinterestApp/WebPhotoPage/Frontend/node_modules/gensync" 3 "PinterestApp" \" -*- nroff -*-
.ad l
.nh
.SH NAME
Dokumentacja katalogu C:/PinterestApp/WebPhotoPage/Frontend/node_modules/gensync
.SH SYNOPSIS
.br
.PP
.SH "Opis szczegółowy"
...
.PP
Take for example \fRfs\&.readFile\fP and \fRfs\&.readFileSync\fP, if you're writing an API that loads a file and then performs a synchronous operation on the data, it can be frustrating to maintain two parallel functions\&.
.SH "Example"
.PP
.PP
.nf
...
.fi
.PP
 or potentially expose one of the async versions as the default, with a \fR\&.sync\fP property on the function to expose the synchronous version\&. 
.PP
.nf
...
.fi
.PP
 js const readFile = function* () { return 42; };

.PP
const readFileAndMore = gensync(function* (){ const val = yield* readFile(); return 42 + val; });

.PP
// In general cases const code = readFileAndMore\&.sync("\&./file\&.js", "utf8"); readFileAndMore\&.async("\&./file\&.js", "utf8")\&.then(code => {}) readFileAndMore\&.errback("\&./file\&.js", "utf8", (err, code) => {});

.PP
// In a generator being called indirectly with \&.sync/\&.async/\&.errback const code = yield* readFileAndMore("\&./file\&.js", "utf8"); 
.PP
.nf
...
.fi
.PP
 js const readFile = gensync({ sync: fs\&.readFileSync, errback: fs\&.readFile, });

.PP
const code = readFile\&.sync("\&./file\&.js", "utf8"); readFile\&.async("\&./file\&.js", "utf8")\&.then(code => {}) readFile\&.errback("\&./file\&.js", "utf8", (err, code) => {}); 
.PP
.nf
...
.fi
.PP
 js const loadFiles = gensync(function* () { return yield* gensync\&.all([ readFile("\&./one\&.js"), readFile("\&./two\&.js"), readFile("\&./three\&.js"), ]); }); 
.PP
.nf
...
.fi
.PP
 js const loadFiles = gensync(function* () { return yield* gensync\&.race([ readFile("\&./one\&.js"), readFile("\&./two\&.js"), readFile("\&./three\&.js"), ]); }); ``` 
